...
class ThreadClassNodeVisitor(private val context: Context, private val node: ClassNode) {
    private lateinit var className: String

    companion object {
        private var TAG: String = "ThreadClassNodeVisitor"
    }

    init {
        TAG = context.extension.name
        className = node.name;
    }
            ...
            method.instructions?.iterator()?.forEach {
                when (it.opcode) {
                    ...
                    Opcodes.ARETURN -> if (method.desc == "L$THREAD;") {
                        method.instructions.insertBefore(it, LdcInsnNode(makeThreadName(className)))
                        ...
                        context.logger.i(" + $SHADOW_THREAD.makeThreadName(Ljava/lang/String;Ljava/lang/String;): ${klass.name}.${method.name}${method.desc}")
                    }
                }
            }
        }
        ...
        if (this.owner == THREAD) {
            ...
            when ("${this.name}${this.desc}") {
                "start()V" -> {
                    ...
                    context.logger.i(" + $SHADOW_THREAD.makeThreadName(Ljava/lang/String;Ljava/lang/String;) => ${this.owner}.${this.name}${this.desc}: ${klass.name}.${method.name}${method.desc}")
                    this.owner = THREAD
                }
    ...
    private fun MethodInsnNode.transformThreadInvokeSpecial(
        ...
        when (this.desc) {
            ...
            "(Ljava/lang/ThreadGroup;Ljava/lang/Runnable;)V" -> {
                ...
                context.logger.i(" + $SHADOW_THREAD.makeThreadName(Ljava/lang/String;Ljava/lang/String;) => ${this.owner}.${this.name}${this.desc}: ${klass.name}.${method.name}${method.desc}")
                context.logger.i(" * ${this.owner}.${this.name}${this.desc} => ${this.owner}.${this.name}$desc: ${klass.name}.${method.name}${method.desc}")
                this.desc = desc
            }
            // Thread(String)
            "(Ljava/lang/String;)V",
            ...
            EXECUTORS -> {
                when (this.name) {
                    "defaultThreadFactory" -> {
                        val r = this.desc.lastIndexOf(')')
                        val desc = "${this.desc.substring(0, r)}Ljava/lang/String;${this.desc.substring(r)}"
                        context.logger.i(" * ${this.owner}.${this.name}${this.desc} => $SHADOW_EXECUTORS.${this.name}$desc: ${klass.name}.${method.name}${method.desc}")
    ...
    private fun TypeInsnNode.transform(klass: ClassNode, method: MethodNode) {
        when (this.desc) {
            /*-*/ HANDLER_THREAD -> this.transformNew(klass, method, SHADOW_HANDLER_THREAD)
            /*---------*/ THREAD -> this.transformNew(klass, method, SHADOW_THREAD)
            THREAD_POOL_EXECUTOR -> this.transformNew(klass, method, SHADOW_THREAD_POOL_EXECUTOR, true)
            /*----------*/ TIMER -> this.transformNew(klass, method, SHADOW_TIMER)
        }
    }
